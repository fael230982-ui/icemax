...
test("platform diagnostics expose readiness catalog and role matrix", async () => {
  ...
  assert.ok(mobileOffline.json().summary.managerReviewRequests >= 1);
  ...
  assert.ok(mobileOffline.json().data.some((item: { requestedFromMobile: boolean }) => item.requestedFromMobile));

  const retryPermissions = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/permissions",
  });
  assert.equal(retryPermissions.statusCode, 200);
  assert.equal(retryPermissions.json().defaultDecision, "deny");
  assert.equal(retryPermissions.json().productionExecutionAllowed, false);
  assert.ok(retryPermissions.json().stages.some((item: { key: string; status: string }) =>
  ...
  const retryActionPlan = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/action-plan",
  });
  assert.equal(retryActionPlan.statusCode, 200);
  assert.equal(retryActionPlan.json().realExecutionAllowed, false);
  assert.equal(retryActionPlan.json().automaticRetryAllowed, false);
  assert.ok(retryActionPlan.json().summary.totalActions >= 3);
  assert.ok(retryActionPlan.json().actions.some((item: { blockedNow: string[] }) =>
    item.blockedNow.includes("real_execution")));

  const retryDailyCommand = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/daily-command",
  });
  assert.equal(retryDailyCommand.statusCode, 200);
  assert.equal(retryDailyCommand.json().realExecutionAllowed, false);
  assert.equal(retryDailyCommand.json().summary.realExecutionBlocked, true);
  assert.ok(retryDailyCommand.json().summary.todayQueue >= 1);
  ...
  const retryEvidencePackage = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/evidence-package",
  });
  assert.equal(retryEvidencePackage.statusCode, 200);
  assert.equal(retryEvidencePackage.json().realExecutionAllowed, false);
  assert.equal(retryEvidencePackage.json().controls.storeRawPayload, false);
  assert.equal(retryEvidencePackage.json().controls.storePayloadHash, true);
  assert.ok(retryEvidencePackage.json().summary.candidates >= 1);
  ...
  const retryProductionReadiness = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/production-readiness",
  });
  assert.equal(retryProductionReadiness.statusCode, 200);
  assert.equal(retryProductionReadiness.json().realExecutionAllowed, false);
  assert.equal(retryProductionReadiness.json().releaseDecision.decision, "keep_blocked");
  assert.ok(retryProductionReadiness.json().readinessScore < 95);
  ...
  const retryWhitelabelContinuousImprovement = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/whitelabel-continuous-improvement",
  });
  assert.equal(retryWhitelabelContinuousImprovement.statusCode, 200);
  assert.equal(retryWhitelabelContinuousImprovement.json().realExecutionAllowed, false);
  assert.equal(retryWhitelabelContinuousImprovement.json().summary.firstTenant, "ICEMAX");
  assert.equal(retryWhitelabelContinuousImprovement.json().summary.scaleAllowed, false);
  assert.ok(retryWhitelabelContinuousImprovement.json().improvementTracks.some((item: { key: string }) =>
  ...
  const retryWhitelabelSecondTenantPreOnboarding = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/whitelabel-second-tenant-pre-onboarding",
  });
  assert.equal(retryWhitelabelSecondTenantPreOnboarding.statusCode, 200);
  assert.equal(retryWhitelabelSecondTenantPreOnboarding.json().realExecutionAllowed, false);
  assert.equal(retryWhitelabelSecondTenantPreOnboarding.json().summary.candidateTenantAllowed, false);
  assert.equal(retryWhitelabelSecondTenantPreOnboarding.json().intakePolicy.collectPublicSecretsAllowed, false);
  assert.ok(retryWhitelabelSecondTenantPreOnboarding.json().preOnboardingSections.some((item: { key: string }) =>
  ...
  const retryWhitelabelPartnerGoLiveAcceptance = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/whitelabel-partner-go-live-acceptance",
  });
  assert.equal(retryWhitelabelPartnerGoLiveAcceptance.statusCode, 200);
  assert.equal(retryWhitelabelPartnerGoLiveAcceptance.json().realExecutionAllowed, false);
  assert.equal(retryWhitelabelPartnerGoLiveAcceptance.json().summary.partnerGoLiveAllowed, false);
  assert.equal(retryWhitelabelPartnerGoLiveAcceptance.json().acceptancePolicy.ownerFinalAcceptanceRequired, true);
  assert.ok(retryWhitelabelPartnerGoLiveAcceptance.json().acceptanceChecks.some((item: { key: string }) =>
  ...
  const retryWhitelabelMorningCommand = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/whitelabel-morning-command",
  });
  assert.equal(retryWhitelabelMorningCommand.statusCode, 200);
  assert.equal(retryWhitelabelMorningCommand.json().realExecutionAllowed, false);
  assert.equal(retryWhitelabelMorningCommand.json().summary.sourceProjectPercent, 86);
  assert.equal(retryWhitelabelMorningCommand.json().summary.plannedProjectPercent, 87);
  assert.equal(retryWhitelabelMorningCommand.json().operatingRules.keepSecretsOutOfRepository, true);
  assert.ok(retryWhitelabelMorningCommand.json().workstreams.some((item: { key: string }) =>
    item.key === "mobile_field_experience"));
  assert.ok(retryWhitelabelMorningCommand.json().blockedActions.includes("enable_real_provider_calls"));

  const retryWhitelabelProductionExecutionMatrix = await app.inject({
    method: "GET",
    url: "/platform/mobile-offline-escalations/whitelabel-production-execution-matrix",
  });
  assert.equal(retryWhitelabelProductionExecutionMatrix.statusCode, 200);
  assert.equal(retryWhitelabelProductionExecutionMatrix.json().realExecutionAllowed, false);
  assert.equal(retryWhitelabelProductionExecutionMatrix.json().summary.projectPercentAfterBlock, 88);
  assert.equal(retryWhitelabelProductionExecutionMatrix.json().releaseRules.realProviderCallsAllowed, false);
  assert.ok(retryWhitelabelProductionExecutionMatrix.json().executionCells.some((item: { key: string; decision: string }) =>
    item.key === "provider_integrations" && item.decision === "blocked"));
  assert.ok(retryWhitelabelProductionExecutionMatrix.json().blockedActions.includes("migrate_real_data_without_tenant_isolation"));
  ...
  assert.equal(offlineTimeline.statusCode, 200);
  assert.equal(offlineTimeline.json().summary.blocked, 1);
  assert.ok(offlineTimeline.json().events.some((item: { key: string }) => item.key === "dry_run"));
